 ...
 * v2.0 - Corrigido Socket.IO para usar /api/ws
 */
...
(function() {
  ...
  // Obter o domínio correto
  // Fazer uma requisição ao servidor para obter o domínio público correto
  let socketOrigin = window.location.protocol + '//' + window.location.hostname + (window.location.port ? ':' + window.location.port : '');
  
  // Tentar obter o domínio correto via API
  async function initializeDomain() {
    try {
      const response = await fetch('/api/trpc/cliente.getDomain');
      const data = await response.json();
      if (data.result && data.result.data && data.result.data.json) {
        const { domain, protocol } = data.result.data.json;
        socketOrigin = protocol + '://' + domain + (window.location.port ? ':' + window.location.port : '');
        console.log('[BRIDGE] Domínio obtido via API:', socketOrigin);
        // Conectar o Socket.IO com o domínio correto
        connectSocket();
      } else {
        console.log('[BRIDGE] Resposta inválida da API:', data);
        connectSocket();
      }
    } catch (err) {
      console.log('[BRIDGE] Erro ao obter domínio via API:', err);
      // Fallback: conectar com o hostname atual
      connectSocket();
    }
  }
  ...
  function connectSocket() {
    ...
    console.log('[BRIDGE] Socket.IO conectando a:', socketOrigin, 'com path: /api/ws');
